 ...
 *
 * The value is rendered centered and `multiline`, capped at AMOUNT_MAX_LINES lines:
 * it first shrinks (base -> floor) to keep the value on one line as it grows, and
 * only once the floor size can't fit it on one line does it wrap across up to that
 * many lines (see `computeAmountFontFit`).
 *
 * Sizing modes:
 * - Self-measure (default): the input measures its own column via `onLayout` and
 *   applies the shared ladder. Callers MUST give it a parent-determined width
 *   (`flex: 1` in a row, `alignSelf: 'stretch'` in a centered column); otherwise the
 *   input sizes to its content and the font-scaling measurement feeds back on itself.
 * - Controlled (`fontSize` prop): the parent computes the size and owns any layout
 *   changes. Used by the Send screen, which measures a stable, layout-independent
 *   width so its row->column token-box switch doesn't cause the fit to oscillate.
 *
 * @param {Object} props
...
const AmountTextInput = forwardRef((props, ref) => {
  ...
  // `singleLine` callers (the Send screen's inline state) get a fixed one-line
  // box so it can never grow to two lines and ride up when centered beside the
  // token box (iOS ignores textAlignVertical on multiline inputs). Wrapping
  // callers grow up to AMOUNT_MAX_LINES.
