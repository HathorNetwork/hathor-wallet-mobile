...
class PinScreen extends React.Component {
  ...
  handleDataMigration = (pin) => {
    ...
    if (accessData !== null &&
        accessData.xpubkey === undefined &&
        accessData.mainKey !== undefined) {
      // Two situations are handled here:
      // 1. From v0.12.0 to v0.13.0 of the lib, xpubkey has changed from wallet:data to wallet:accessData.
      // So if the user is still in an app version before v0.13.0, we can't use xpubkey directly from accessData
      //
      // 2. When the user updated the app to the newest version directly from a version before we've executed the xpubkey migration
      // In that case we have deleted the user walletData before migrating the xpubkey to the accessData
  ...
  dismiss = (pin) => {
    if (this.props.isLockScreen) {
      // in case it's the lock screen, we just have to execute the data migration method an change redux state.
      // No need to execute callback or go back on navigation
